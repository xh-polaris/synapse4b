// Code generated by hertz generator.

package synapse4b

import (
	"context"

	"github.com/cloudwego/hertz/pkg/app"
	"github.com/cloudwego/hertz/pkg/protocol/consts"
	basicuser "github.com/xh-polaris/synapse4b/biz/api/model/basicuser"
	buapp "github.com/xh-polaris/synapse4b/biz/application/basicuser"
)

// BasicUserRegister .
// @router /basic_user/register [POST]
func BasicUserRegister(ctx context.Context, c *app.RequestContext) {
	var err error
	var req basicuser.BasicUserRegisterReq
	err = c.BindAndValidate(&req)
	if err != nil {
		invalidParamRequestResponse(c, err.Error())
		return
	}

	resp, err := buapp.BasicUserSVC.RegisterNewBasicUser(ctx, &req)
	if err != nil {
		internalServerErrorResponse(ctx, c, err)
		return
	}
	c.JSON(consts.StatusOK, resp)
}

// BasicUserLogin .
// @router /basic_user/login [POST]
func BasicUserLogin(ctx context.Context, c *app.RequestContext) {
	var err error
	var req basicuser.BasicUserLoginReq
	err = c.BindAndValidate(&req)
	if err != nil {
		invalidParamRequestResponse(c, err.Error())
		return
	}

	resp, err := buapp.BasicUserSVC.Login(ctx, &req)
	if err != nil {
		internalServerErrorResponse(ctx, c, err)
		return
	}
	c.JSON(consts.StatusOK, resp)
}

// BasicUserResetPassword .
// @router /basic_user/reset_password [POST]
func BasicUserResetPassword(ctx context.Context, c *app.RequestContext) {
	var err error
	var req basicuser.BasicUserResetPasswordReq
	err = c.BindAndValidate(&req)
	if err != nil {
		invalidParamRequestResponse(c, err.Error())
		return
	}

	resp, err := buapp.BasicUserSVC.ResetPassword(ctx, &req)
	if err != nil {
		internalServerErrorResponse(ctx, c, err)
		return
	}
	c.JSON(consts.StatusOK, resp)
}

// CreateBasicUser .
// @router /basic_user/create [POST]
func CreateBasicUser(ctx context.Context, c *app.RequestContext) {
	var err error
	var req basicuser.BasicUserCreateReq
	err = c.BindAndValidate(&req)
	if err != nil {
		c.String(consts.StatusBadRequest, err.Error())
		return
	}

	resp, err := buapp.BasicUserSVC.CreateBasicUser(ctx, &req)
	if err != nil {
		internalServerErrorResponse(ctx, c, err)
		return
	}
	c.JSON(consts.StatusOK, resp)
}

// QueryUnit .
// @router /unit/query [POST]
func QueryUnit(ctx context.Context, c *app.RequestContext) {
	var err error
	var req basicuser.QueryUnitReq
	err = c.BindAndValidate(&req)
	if err != nil {
		c.String(consts.StatusBadRequest, err.Error())
		return
	}

	resp, err := buapp.BasicUserSVC.QueryUnit(ctx, &req)
	if err != nil {
		internalServerErrorResponse(ctx, c, err)
		return
	}
	c.JSON(consts.StatusOK, resp)
}

// CreateUnit .
// @router /unit/create [POST]
func CreateUnit(ctx context.Context, c *app.RequestContext) {
	var err error
	var req basicuser.CreateUnitReq
	err = c.BindAndValidate(&req)
	if err != nil {
		c.String(consts.StatusBadRequest, err.Error())
		return
	}

	resp, err := buapp.BasicUserSVC.CreateUnit(ctx, &req)
	if err != nil {
		internalServerErrorResponse(ctx, c, err)
		return
	}
	c.JSON(consts.StatusOK, resp)
}

// GetUnit .
// @router /unit/get [POST]
func GetUnit(ctx context.Context, c *app.RequestContext) {
	var err error
	var req basicuser.GetUnitReq
	err = c.BindAndValidate(&req)
	if err != nil {
		c.String(consts.StatusBadRequest, err.Error())
		return
	}

	resp, err := buapp.BasicUserSVC.GetUnit(ctx, &req)
	if err != nil {
		internalServerErrorResponse(ctx, c, err)
		return
	}

	c.JSON(consts.StatusOK, resp)
}
